...
// Ensure JSZip is available from the global scope (loaded via CDN)
...
const App = () => {
  ...
  // Fix: Add explicit types to fix destructuring error from an untyped Promise.
  const fileToGenerativePart = (file: File) => {
    return new Promise<{ part: { inlineData: { mimeType: string, data: string } }, url: string }>((resolve, reject) => {
  ...
  const analyzeResume = async (file: File) => {
    try {
      const { part, url } = await fileToGenerativePart(file);
      ...
      return { data: { ...parsedData, id: file.name + Date.now(), matchScore: null }, url };
    } catch (e) {
  ...
  const handleFileDrop = useCallback(async (files) => {
    ...
    for (let i = 0; i < allFiles.length; i++) {
        ...
        if (analysisResult) {
            analyzedResumes.push(analysisResult);
            const { data, url } = analysisResult;
            const resumeWithUrl = { ...data, fileURL: url };
    ...
    const duplicateCount = analyzedResumes.length - uniqueCount;
      ...
      <div class="control-panel">
        ...
        <div class="control-section">
            ...
            <textarea 
                ...
                disabled=${isLoading}
            ></textarea>
            <button onClick=${handleMatch} disabled=${isLoading || resumes.length === 0}>
                ${T.matchButton}
            </button>
        </div>
